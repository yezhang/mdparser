import * as runtime from 'runtime/antlr4/index.web.js';
import CharsAsTokens from 'parser/CharsAsTokens';
import KdsMarkdownParser from 'parser/KdsMarkdownParser';
...
describe("MarkdownParser", () => {
    xtest("#header1", () => {
        let input = '#header1';
        const chars = new runtime.InputStream(input);
        const charTokens = new CharsAsTokens(chars, KdsMarkdownParser.tokenNames);
        const tokens = new runtime.CommonTokenStream(charTokens);
        const parser = new KdsMarkdownParser(tokens);
        const t = parser.file();
    });
...
`;
        ...
        test('remarkable parse', () => {
            let md = new Remarkable('commonmark');
            md.parse(content, {});
        });

        test('markdown-it parse', () => {
...
// const errorListener = new VuePartsErrorListener();
// const chars = new antlr4.InputStream(input);
// const lexer = new VuePartsLexerMergable(chars);
// lexer.removeErrorListeners();
// lexer.addErrorListener(errorListener);
// const tokens = new antlr4.CommonTokenStream(lexer);
// const parser = new VuePartsParser(tokens);
// parser.removeErrorListeners();
// parser.addErrorListener(errorListener);

// parser.buildParseTrees = true;
// const parseTree = parser.sfc();
